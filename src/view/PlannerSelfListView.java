 ...
 * @author xuhang
 */
public class PlannerSelfListView extends javax.swing.JFrame {
	 ...
	 */
	public PlannerSelfListView(InsuranceSystem system) {
	...
	private void initTable(){
		...
		for (int i=0;i<plans.size();i++) {
			...
			if(!plan.getCreateUser().getUsername().equals(system.getCurrentUser().getUsername())) {
				continue;
			}
			showPlans.add(plan);
		...
		delete.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if(planIndex!=-1) {
					List<Plan> plans=system.getPlanDirectory().getPlanList();
					for(Plan plan:plans) {
						if(plan.getPlanId()==showPlans.get(planIndex).getPlanId()) {
							plans.remove(plan);
							break;
						}
					}
